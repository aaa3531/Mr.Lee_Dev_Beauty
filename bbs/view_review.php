...
if ($w == 'rvse'){
    ...
    if($mb_1){
        $sql_mem = "select * from g5_member where mb_1 = '$mb_1'";
        //echo  "select * from g5_member where mb_1 = '$mb_1'";
    }else if($mb_1 && $mb_3){
        $sql_mem = "select * from g5_member where mb_1 = '$mb_1' and mb_3 = '$mb_3'";
        //echo "select * from g5_member where mb_1 = '$mb_1' and mb_3 = '$mb_3'";
    }else if(empty($mb_1) && $mb_3){
         $sql_mem = "select * from g5_member where mb_3 = '$mb_3'";
         //echo "select * from g5_member where mb_3 = '$mb_3'";
    }
    $result_m = sql_query($sql_mem);
    while($row = sql_fetch_array($result_m)){

//     echo $row['mb_id'];
//     echo '<br>';
// echo  "select * from $write_table where wr_parent ='$wr_parent' and wr_is_comment = '1' and mb_id = '{$list[$i]['mb_id']}' and wr_subject='$wr_subject' and wr_1 = '$wr_1'";
//        echo '<br>';
    if(!$wr_1){
         $sql = " select * from $write_table where wr_parent = '$wr_id' and mb_id = '{$row['mb_id']}' and wr_is_comment = 1 order by wr_comment, wr_comment_reply ";
         //echo " select * from $write_table where wr_parent = '$wr_id' and mb_id = '{$row['mb_id']}' and wr_is_comment = 1 order by wr_comment, wr_comment_reply ";
    }else if($wr_1 > 0){
        $sql = " select * from $write_table where wr_parent = '$wr_id' and wr_1 = '$wr_1' and wr_is_comment = 1 order by wr_comment, wr_comment_reply ";
         //echo " select * from $write_table where wr_parent = '$wr_id' and wr_1 = '$wr_1' and wr_is_comment = 1 order by wr_comment, wr_comment_reply ";
    } 
...
//    $list = array();
   ...
   //goto_url(G5_BBS_URL.'/view_review.php?bo_table='.$bo_table.'&wr_parent='.$wr_parent.'&wr_id='.$wr_parent);  
}else{
    $list = array();
     echo 'test3';
    echo $wss;
...
include_once($board_skin_path.'/view_comment_all.skin.php');
